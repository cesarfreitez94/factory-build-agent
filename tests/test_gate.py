...
class TestTestingReviewGates:
    ...
    def test_init_creates_ci_cd_gate(self, tmp_path):
        runner = CliRunner()
        result = runner.invoke(main, ["init", "-d", str(tmp_path)])
        assert result.exit_code == 0

        state = json.loads((tmp_path / ".factory" / "state.json").read_text())
        assert "ci_cd" in state["gates"]
        assert state["gates"]["ci_cd"]["owner_agent"] == "cicd_manager"
        assert len(state["gates"]["ci_cd"]["rules"]) == 3
