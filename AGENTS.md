...
- **OpenCode local plugin**: `.opencode/plugins/fba-agent-observer.ts` observes only framework
  agents in `.opencode/agents`, excluding `templates/.opencode/agents`, and writes per-session
  observability reports to `.factory/observability/`.
- **Aux CLI**: Python 3.11+ (Click library) for the `fba` command-line tool.

### Plugins

El framework soporta un sistema de plugins locales de OpenCode para extender su capacidad de observabilidad y trazabilidad. Los plugins son archivos TypeScript que se cargan desde `.opencode/plugins/`.

- **Plugin de Agent Observer** (`.opencode/plugins/fba-agent-observer.ts`): Observa agentes meta del framework en `.opencode/agents/`, registra hashes de prompts, tokens/costo por agente, invocaciones agente-a-agente, herramientas y acceso a archivos observables. Genera reportes Markdown y JSONL por sesion en `.factory/observability/`. Excluye `templates/.opencode/agents` del analisis.

**Canal flexible para extensiones**: Ver [M23: CAFL-Plugins](../ROADMAP.md#m23-cafl-plugins) para ideas de plugins sin roadmap estricto.
...
- `.factory/module_registry.json` — Lightweight registry consumed by `SchemaManager` to resolve Odoo model ownership.
- `.factory/registry_index.json` — Deep autoindex of existing Odoo addons generated by `fba registry index`; contains models, fields, views, controllers, reports, security, data/demo files, crons, wizards, and OWL components for planner/constructor/reviewer context.
- `.factory/observability/` — Local OpenCode runtime telemetry generated by the FBA agent observer plugin; includes `agent-index.json`, per-session JSONL ledgers, and Markdown reports with per-agent token/cost attribution.
...
- Semantic graph: M17 introduces the closed ontology and schema for `.factory/graph.json`
  plus `fba graph validate`; later M17 feats add persistence, queries, and agent emission.
...
| Semantic Memory | M17 graph ontology | Closed NodeType/EdgeType ontology with `schemas/graph.schema.json` and `fba graph validate`; persistence/query manager follows in M17 |
...
- **M16: Foundation Intelligence** — IN PROGRESS. ModuleRegistry autoindexado, Odoo Pattern Knowledge Base, version-aware Odoo knowledge.
- **M17: Semantic Core** — IN PROGRESS. Typed semantic graph, ontology, graph queries, agent graph emission, methodology stack.
